...
var _ = self.Prism = {
  ...
  tokenize: function(text, grammar, language) {
    ...
    tokenloop: for (var token in grammar) {
      ...
      for (var i=0; i<strarr.length; i++) { // 不要在循环期间缓存长度
        
        var str = strarr[i];
        
        if (strarr.length > text.length) {
          // 有些事情非常糟糕，ABORT，ABORT！
          break tokenloop;
        }
...
/*!
 * headroom.js v0.3.11 - 给你的页面一些空间。 隐藏您的标题，直到您需要它
...
(function(global) {
  ...
  /**
   * 通过requestAnimationFrame处理事件的debouncing
   * @see http://www.html5rocks.com/en/tutorials/speed/animations/
   * @param {Function} callback 处理任何事件的回调
   */
...
   * 向上滚动时显示标题
   * @constructor
   * @param {DOMElement} elem 标题元素
   * @param {Object} options 小部件的选项
   */
...
      // 可能恢复上一个滚动位置
    ...
    update : function() {
      ...
      if(currentScrollY < 0) { // 忽略OSX中的弹性滚动
        return;
      }
